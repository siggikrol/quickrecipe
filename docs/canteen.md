...
- **Automatic allergens:** A bundled ingredient database supplies allergen information for every ingredient in the recipe collection. No checkboxes or Save step are required. Week cards use a clickable red badge for identified allergens or an amber badge for possible/incomplete information. These colors describe certainty, not reaction severity. Opening the badge shows the specific allergens. Allergen details are available from the week badges; Recipe settings contains only portion settings and base-recipe links. Known allergens, possible allergens that vary by product, and ingredients absent from the database remain distinct. Saved declarations from earlier versions are retained.
...
`ingredientAllergens` is an adjunct to existing ingredient tuples, keyed by their exact original name within that recipe. It is ingredient-level declaration data, independent of any Canteen plan. An absent entry uses `ingredient-allergens.js`; an existing array remains a saved declaration, including an explicitly empty array. The database is shipped and cached with the app, rather than written into individual recipes. It groups exact ingredient names into profiles with `contains`, `possible`, and `productDependent` fields. Unknown custom ingredients are flagged rather than silently treated as allergen-free. Changing language does not change the underlying lookup. Add new ingredient names to the database when extending the recipe collection; coverage tests enforce this. The 14 supported IDs are gluten, crustaceans, eggs, fish, peanuts, soybeans, milk, nuts, celery, mustard, sesame, sulphites, lupin and molluscs.

Existing recipe adjustments apply after ingredient and foundation inheritance; explicit additions win if the same allergen is also excluded. The current UI does not require allergen editing. Recipe settings save serving counts without rebuilding or discarding saved declarations. Possible allergens propagate through base recipes and appear consistently in menus, Kitchen, recipe details and printouts. Product formulations, sulphite concentrations and cross-contact cannot be established from a generic ingredient name; product labels take precedence over automatic estimates. No output claims allergy safety.
...
Run `npm test`. This includes ingredient-database coverage, planning-model, shared-calculation and Canteen UI suites alongside the existing recipe suites. Coverage includes zero overrides, decimal scales, nested foundations/cycles, compatible/incompatible units, ranges, unknown yields, allergens, batches, copying, reloads, translations, print structure and restoring recipe selection. Version 75 aligns app, HTML assets and offline caching, including all new modules.
